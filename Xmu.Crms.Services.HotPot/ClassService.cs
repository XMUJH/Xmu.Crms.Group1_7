...
namespace Xmu.Crms.Services.HotPot
{
    public class ClassService :IClassService
    {
        ...
        public long InsertCourseSelectionById(long userId, long classId)//测试成功
        {
            ...
            if (isExist.Any())
            {
                throw new Xmu.Crms.Shared.Exceptions.InvalidOperationException();
            }
        ...
        {
            if (courseId <= 0) throw new ArgumentException("格式错误！");
            Course course = _db.Course.Find(courseId);
            classInfo.Course = course;
            IList<ClassInfo> classes = _db.ClassInfo.ToList();
            classInfo.Id = classes.Count + 1;
        ...
        */
        ...
        public IList<ClassInfo> ListClassByUserId(long userId)//测试成功
        {
            if (userId < 0)
                throw new ArgumentException("UserID格式错误");
