...
namespace Xmu.Crms.Services.Group1_7
{
    public class CourseService : ICourseService
    {
        ...
        /// <summary>
        /// 按课程名称和教师名称获取班级列表.
        /// 根据课程名和教师名获取课程ID，通过课程ID获取班级列表;若课程名和班级名均不为空，取交集
        /// @author yexiaona
        /// <summary>
        /// <param name="courseName">课程名称</param>
        /// <param name="teacherName">教师名称</param>
        /// <returns>list 班级列表</returns>
        /// <exception cref="T:System.UserNotFoundException">无此姓名的教师</exception>
        /// <exception cref="T:System.CourseNotFoundException">无此名称的课程</exception>
        /// <seealso cref="M:Xmu.Crms.Shared.Service.ICourseService.listClassByCourseName(System.String)"/>
        /// <seealso cref="M:Xmu.Crms.Shared.Service.ICourseService.listClassByTeacherName(System.String)"/>
        public IList<ClassInfo> ListClassByName(string courseName, string teacherName)
        {
            if (_db.Course.Where(c => c.Name == courseName) == null)
                throw new CourseNotFoundException("无此名称的课程");
            if (_db.UserInfo.SingleOrDefault(c => c.Name == teacherName) == null)
                throw new UserNotFoundException("无此姓名的教师");

            UserInfo teacher = _db.UserInfo.SingleOrDefault(c => c.Name == teacherName);
            Course course = _db.Course.Include(c => c.Teacher).SingleOrDefault(c => c.Name == courseName && c.Teacher == teacher);
            List<ClassInfo> classes = _db.ClassInfo.Include(c => c.Course).Where(c => c.Course.Id == course.Id).ToList();
